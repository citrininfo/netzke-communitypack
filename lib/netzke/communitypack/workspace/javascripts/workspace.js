{
  initComponent: function() {
    ...
    this.on('tabchange', function(panel, tab) {
      if (tab.dynamicName == 'cmp0' && this.alwaysReloadFirstTab) {
        tab.removeAll();
      }
  ...
  */
  loadInTab: function(cmp, options) {
    ...
    if (options.newTab) {
      var lastTabIndex = parseInt(this.items.last().dynamicName.replace("cmp", ""));
    ...
    params.dynname = cmpName;

    this.netzkeLoadComponent(cmp, {container: receivingTab, params: params, callback: function(c) {
      receivingTab.setTitle(c.title);
      receivingTab.dynamicName = c.dynamicName;
    }});
    return receivingTab;
  }
}
